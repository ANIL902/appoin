...
export class AppointmentsComponent implements OnInit {
  ...
  toggleAllSelection(type){console.log(type)
    if(type =='floor') {
      ...
      if (this.allServiceSelected.selected) {
        ...
        .patchValue([... this.serviceList.map(item => item), 0]);
  ...
  loadDepartments(floors){console.log('selected floors : ', floors);
   ...
   this.floorName = ''
    ...
    let findex = this.levellist.findIndex(y => y.floorId == userSelected[0]);
...
loadServices(departments){
  if(departments.length>0){
    ...
    if(deptSelected.length>0){
      ...
      console.log("findex=>",findex)
      if (findex != -1) {
        this.deptName = this.filterDeptGroup[findex].deptName;
...
selectedModule(services){
  ...
  if (services.length> 0) {
    ...
    if (serviceSelected.length > 0) {
     let findex = services.findIndex(y => y == serviceSelected[0]);
     console.log("findex=>",findex)
     if (findex != -1) {
        this.servicename = services[0].serviceEngName;
     }
    }
